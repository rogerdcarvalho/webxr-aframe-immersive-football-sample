AFRAME.registerComponent("player-move", {
    ...
    init: function() 
    {
        ...
        // other components may set this value to enable/disable this component and determine the maximum distance
        this.enabled = true;
        if (window.boundaries) {
            console.log("window.bounds found")
            this.maxLeft = window.boundaries.left || -1 * Infinity
            this.maxRight = window.boundaries.right || Infinity
            this.maxUp = window.boundaries.up || Infinity
            this.maxDown = window.boundaries.down ||  -1 * Infinity
            this.maxForward = window.boundaries.forward || Infinity
            this.maxBackward = window.boundaries.backward || -1 * Infinity
        } else {
            this.maxLeft = -1 * Infinity
            this.maxRight = Infinity
            this.maxUp =  Infinity
            this.maxDown = -1 * Infinity
            this.maxForward = Infinity
            this.maxBackward = -1 * Infinity
        }
       

        // Get all static elements
        this.staticElements = document.querySelectorAll(`[physx-body="type: static;"]`);
        console.log(this.staticElements);
    ...
    tick: function()
    {
        ...
        // Block camera moving through static objects
             ...
             !this.controllerData.leftGrip.pressing )
        {
            ...
            window.debug= "Moving right, forward to " + this.el.object3D.position.x + this.moveRight + ", " + this.el.object3D.position.z + this.moveForward 
            if (this.el.object3D.position.x + this.moveRight > this.maxLeft &&
                this.el.object3D.position.x + this.moveRight < this.maxRight) 
                {
                    this.el.object3D.position.x = this.el.object3D.position.x + this.moveRight;
                } else {
                    window.debug = " Horizontal move blocked due to boundaries"
                }
            if (this.el.object3D.position.z + this.moveForward > this.maxBackward &&
                this.el.object3D.position.z + this.moveForward < this.maxForward) 
                {
                    this.el.object3D.position.z = this.el.object3D.position.z + this.moveForward;
                } else {
                    window.debug = "Z move blocked due to boundaries"
                }
        }
        ...
        if ( this.data.motionEnabled && this.turnReady &&
             ((this.controllerData.leftGrip.pressing && Math.abs(this.leftX) > 0.90))
           )
        {
             ...
             Math.abs(this.controllerData.leftAxisY) > 0.25 )
        {
            ...
            window.debug = "Moving vertically to " + this.el.object3D.position.y + this.moveDistance
            if (this.el.object3D.position.y + this.moveDistance > this.maxDown &&
                this.el.object3D.position.y + this.moveDistance < this.maxUp) 
                {
                    this.el.object3D.position.y = this.el.object3D.position.y + this.moveDistance;
                } else {
                    window.debug = "Vertical move blocked due to boundaries"
                }
        }
    }
});
